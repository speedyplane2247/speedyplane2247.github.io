...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		Game.getNewTicker=function()
		{
			...
			if (list.length==0)
			{
				if (Game.cookiesEarned<1) list.push("CCAntiCheat v2.11; AC Status: Working!");
			}
		...
		order=30200;
		new Game.Achievement('Anti-Cheat Trigger','You have cheated!.',[10,6]);Game.last.pool='shadow';
		order=11010;
		...
		if (!Game.OnAscend && Game.AscendTimer==0)
		{
			...
			if (Game.T%(Game.fps*5)==0 && Game.ObjectsById.length>0)//check some achievements and upgrades
			{
				...
				function sleep(ms) {
					...
					// used for anticheat mostly 
				  }
				  
				function triggerAC() {
					// triggers AC, and force bans account.
					Game.cookiesEarned = -483423894284309482304938409384309482309483
					Game.cookies = 4304213804284023843094834092834093284309248908094830493809
				}
				if (Game.cookiesEarned<Game.cookies) {Game.Win('Anticheat Trigger! You have cheated!');
				// anticheat trigger
				Game.bakeryNameSet("Anti-Cheat Trigger: CHEATS DETECTED")
				acstatus = "CHEAT DETECTED!"
				console.error("Banned from Secure Lobby!")
				Game.cookies = 7219423937928742398472398749837412398472394823489327432984723984729178979
				Game.cookiesEarned = -39803840983024830948230484098420948 // ensure AC triggers on next run
				Game.cookiesForfeited = Infinity
				Game.cookiesd = "Banned for Cheating."
				Game.cookiesd = "Banned for Cheating."
				Game.cookiesd = "Banned for Cheating."
				Game.cookiesd = "Banned for Cheating."
				Game.cookiesd = "Banned for Cheating."
				...
				console.error("Banned from Secure Lobby!")
				alert("This account has been terminated for a cheating trigger by CCAC!")
				Game.win("Account banned for cheating until 6/4/1001") // perm ban
	
				var unban = "6/4/1001" // unbans on date, requires login in that date to unban or it goes for another day.
				}
